...
<div class="mb-12 border-b border-gray-100 pb-8">
    ...
    <h1 class="text-3xl font-black text-gray-900 tracking-tight">Modifier <span class="text-[#F53003]">Catégorie</span></h1>
</div>
...
<div class="bg-white rounded-[2.5rem] border border-gray-100 p-10 shadow-sm max-w-3xl">
    <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-8">
        ...
        <div>
            ...
            <input type="text" name="name" placeholder="Nom de la catégorie..." class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 text-gray-900 font-black text-xl focus:ring-2 focus:ring-[#F53003]/20 focus:border-[#F53003] transition-all placeholder-gray-200" value="{{ old('name', $category->name) }}" required>
            @error('name')<span class="text-red-500 text-xs font-bold mt-2 block">{{ $message }}</span>@enderror
        </div>

        <div>
            <label class="block text-xs font-black text-gray-500 uppercase tracking-[0.2em] mb-4 border-l-4 border-gray-200 pl-4">Description</label>
            <textarea name="description" rows="5" placeholder="Décrivez brièvement cette catégorie..." class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 text-gray-900 font-bold focus:ring-2 focus:ring-[#F53003]/20 focus:border-[#F53003] transition-all placeholder-gray-200 leading-relaxed">{{ old('description', $category->description) }}</textarea>
